...
function disle_theme_setup() {

	// Make theme available for translation.
	load_theme_textdomain( 'disle', get_template_directory() . '/languages' );
	...
	add_image_size( 'disle-post-standard', 1170, 650, true );
	add_image_size( 'disle-project-standard', 1170, 480, true );

	// Register menus
	register_nav_menu( 'primary', esc_html__( 'Primary Menu', 'disle' ) );
...
add_action( 'after_setup_theme', 'disle_theme_setup' );

// Enqueues scripts and styles.
function disle_theme_scripts() {
	...
	// Theme Style
	wp_enqueue_style( 'disle-theme-style', get_stylesheet_uri(), array(), '1.0' );
	wp_add_inline_style( 'disle-theme-style', apply_filters( 'disle_custom_colors_css', null ) );

	// Theme Script
	wp_enqueue_script( 'disle-theme-script', get_template_directory_uri() . '/assets/js/main.js', array( 'jquery' ), '1.0', true );

	// Carousel
	$post_format = get_post_format();
	if ( (is_single() && ( 'post-gallery' == $post_format )) ||
		(is_singular('post') && disle_get_mod( 'blog_single_related', false )) ||
		(is_singular('project') && disle_get_mod( 'project_related', false )) ) {
		...
		wp_enqueue_script( 'disle-slide', get_template_directory_uri() . '/assets/js/slide.js', array('jquery'), '1.0.0', true );
	}
...
add_action( 'wp_enqueue_scripts', 'disle_theme_scripts' );

// Registers a widget areas.
function disle_sidebars_init() {
	// Sidebar for Blog
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar Blog', 'disle' ),
		'id'            => 'sidebar-blog',
		'description'   => esc_html__( 'Add widgets here to appear in Sidebar Blog.', 'disle' ),
	...
	// Sidebar for Pages
	register_sidebar( array(
		'name'			=> esc_html__( 'Sidebar Page', 'disle' ),
		'id'			=> 'sidebar-page',
		'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Page', 'disle' ),
		'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
		'after_widget'	=> '</div>',
		'before_title'	=> '<h2 class="widget-title"><span>',
		'after_title'	=> '</span></h2>'
	...
	if ( post_type_exists('project') ) {
		...
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Portfolio', 'disle' ),
			'id'			=> 'sidebar-portfolio',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Portfolio', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
	...
	if ( post_type_exists('service') ) {
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Service', 'disle' ),
			'id'			=> 'sidebar-service',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Service', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
	...
	if ( class_exists( 'woocommerce' ) ) {
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Shop', 'disle' ),
			'id'			=> 'sidebar-shop',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Shop', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
	...
	if (class_exists('Give')) {
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Cause', 'disle' ),
			'id'			=> 'sidebar-give',
			'description'	=> esc_html__( 'Add widgets here to appear in Cause Single Sidebar', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
	...
	if ( disle_footer_style() == '1' ) {
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Footer 1', 'disle' ),
			'id'			=> 'sidebar-footer-1',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Footer 1', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
		...
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Footer 2', 'disle' ),
			'id'			=> 'sidebar-footer-2',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Footer 2', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
		...
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Footer 3', 'disle' ),
			'id'			=> 'sidebar-footer-3',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Footer 3', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
		...
		register_sidebar( array(
			'name'			=> esc_html__( 'Sidebar Footer 4', 'disle' ),
			'id'			=> 'sidebar-footer-4',
			'description'	=> esc_html__( 'Add widgets here to appear in Sidebar Footer 4', 'disle' ),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h2 class="widget-title"><span>',
			'after_title'	=> '</span></h2>'
...
add_action( 'widgets_init', 'disle_sidebars_init' );
...
$url = 'https://tplabs.co/api/checkUpdate?theme=disle&code=' . $code . '&web=' . $web;
